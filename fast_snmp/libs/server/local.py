...
class LocalServer(ExecuteCommand):
    ...
    def ping(self, host: str) -> bool:
        try:
            ...
            response = subprocess.run(
                f"{ping_command} -V", shell=True, capture_output=True, text=True
            )
            if response.stderr:
                response = subprocess.run(
                    f"{ping_command} {host}", shell=True, capture_output=True, text=True
                )
                if response.stderr:
                    raise Exception(response.stderr)
                else:
                    if "is alive" in response.stdout:
                        return True
                    else:
                        return False
            else:
                ...
                if response.stderr:
                    ...
                    if "1 received, 0% packet loss" in response.stdout:
                        return True
                    else:
                        return False
        except Exception as error:
