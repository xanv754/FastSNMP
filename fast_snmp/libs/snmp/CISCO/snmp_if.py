...
from fast_snmp.constants import SNMPIF_MIB, HeaderResponseInterfacesSNMP, response_interface_snmp, response_snmp
...
class SNMPIF:
    ...
    def get_ifIndex(device: Device) -> pd.DataFrame:
        try:
            ...
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFINDEX, type_response=oid_type, date_time=device.date)
            response = response[[
    ...
    def get_ifNumber(device: Device) -> pd.DataFrame:
        try:
            ...
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFNUMBER, type_response=oid_type, date_time=device.date)
            response = response[[
                HeaderResponseInterfacesSNMP.HOST, 
    ...
    def get_ifName(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFNAME, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifAlias(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFALIAS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifDescr(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFDESCR, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifHighSpeed(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHIGHSPEED, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifAdminStatus(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFADMINSTATUS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifOperStatus(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFOPERSTATUS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifCounterDiscontinuityTime(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFCOUNTERDISCONTINUITYTIME, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifCounterDiscontinuityTime - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifHCInOctets(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCINOCTETS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifHCInUcastPkts(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCINUCASTPKTS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifHCInUcastPkts - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifHCInMulticastPkts(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCINMULTICASTPKTS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifHCInMulticastPkts - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifHCInBroadcastPkts(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCINBROADCASTPKTS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifHCInBroadcastPkts - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifInErrors(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFINERRORS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifInDiscards(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFINDISCARDS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifHCOutOctets(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCOUTOCTETS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifHCOutUcastPkts(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCOUTUCASTPKTS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifHCOutUcastPkts - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifHCOutMulticastPkts(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCOUTMULTICASTPKTS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifHCOutMulticastPkts - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifHCOutBroadcastPkts(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFHCOUTBROADCASTPKTS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
            logger.error(f"SNMP IF-MIB error: Failed to obtain ifHCOutBroadcastPkts - {error}")
            return pd.DataFrame(columns=response_interface_snmp)
    ...
    def get_ifOutErrors(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFOUTERRORS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
    ...
    def get_ifOutDiscards(device: Device) -> pd.DataFrame:
        try:
            ...
            if not response: return pd.DataFrame(columns=response_interface_snmp)
            response = TransformSNMP.output(host=device.host, stdout=response, oid=SNMPIF_MIB.IFOUTDISCARDS, type_response=oid_type, date_time=device.date)
            return response
        except Exception as error:
